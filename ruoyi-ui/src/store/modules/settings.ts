...
const useSettingsStore = defineStore(
  'settings',
  {
    ...
    actions: {
      // 未保存布局偏好时，普通用户默认使用顶部菜单，管理员沿用系统默认布局
      applyRoleDefaultNavType(roles: string[]) {
        const layoutSetting = JSON.parse(localStorage.getItem('layout-setting') || '{}') || {}
        if (layoutSetting.navType !== undefined) {
          return
        }
        this.navType = roles.includes('admin') ? navType : USER_DEFAULT_NAV_TYPE
      },
      // 修改布局设置
