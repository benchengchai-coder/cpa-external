         ...
         <el-table-column label="状态" align="center" prop="status" width="100">
            <template #default="scope">
               <el-switch
                  ...
                  inactive-text="关闭"
                  inline-prompt
                  :loading="isStatusChanging(scope.row.noticeId)"
                  :disabled="!checkPermi(['system:notice:edit'])"
...
const statusChangingNoticeIds = ref<number[]>([])
...
/** 公告状态修改 */
function handleStatusChange(row: SysNotice) {
  if (row.noticeId === undefined || !row.noticeTitle || row.status === undefined) {
    row.status = row.status === "0" ? "1" : "0"
    return
  }

  const noticeId = row.noticeId
  const statusText = row.status === "0" ? "启用" : "关闭"
  statusChangingNoticeIds.value.push(noticeId)
  updateNotice({
    noticeId,
    ...
    statusChangingNoticeIds.value = statusChangingNoticeIds.value.filter(id => id !== noticeId)
  })
}

function isStatusChanging(noticeId?: number) {
  return noticeId !== undefined && statusChangingNoticeIds.value.includes(noticeId)
}
