    ...
    protected SecurityFilterChain filterChain(HttpSecurity httpSecurity) throws Exception
    {
        return httpSecurity
            ...
            .authorizeHttpRequests((requests) -> {
                ...
                // 异步与错误分发直接放行，避免流式响应丢失 SecurityContext 后触发二次 Access Denied
                requests.dispatcherTypeMatchers(DispatcherType.ASYNC, DispatcherType.ERROR).permitAll();
                // 对于登录login 注册register 验证码captchaImage 允许匿名访问
                requests.requestMatchers("/login", "/apikey/login", "/register", "/register/status", "/register/email/code",
                                "/password/reset/status", "/password/reset/email/code", "/password/reset", "/captchaImage")
                        .permitAll()
